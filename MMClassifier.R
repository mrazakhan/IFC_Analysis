...
training<-training[,!names(training) %in% c('X','X.2','X.1','CallerId')]

testing_callerid=testing$CallerId

testing<-testing[,!names(testing) %in% c('X','X.2','X.1','CallerId')]
...
#Removing UserType from Training DataFrame
training2<-training[,!names(training) %in% c('UserType')]

write.csv(file='./regressionGLM_NormalMM.csv',
          x=regressionGLM(training_labels, training2))
